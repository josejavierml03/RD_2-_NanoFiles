...
public class NFControllerLogicDir {
	...
	protected boolean doLogin(String directoryHostname, String nickname) {
		...
		try {
			...
			if(directoryConnector.logIntoDirectory(nickname)) {
				System.out.println("Estado: Success" + "\n" + "Salida: Login Successful" + "\n" + 
						"Key: " + this.directoryConnector.getSessionKey());
				result = true;
			}else {
				System.out.println("Estado: Failure" + "\n" + "Salida: Login Failed");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	protected boolean getAndPrintUserList() {
		...
		if(userlist != null) {
			String usernames = userlist[0];
			for (int i = 1; i < userlist.length; i++) {
				usernames += ",";
				usernames += userlist[i];
			}
			
			System.out.println("Estado: Success" + "\n" + "Salida:"+ usernames);
			result = true;
		}else {
			System.out.println("Estado: Failure" + "\n" + "Salida: Userlist Failed");
		}
	...
	public boolean registerFileServer(int serverPort) {
		...
		boolean result = false;
			...
			if(directoryConnector.registerServerPort(serverPort)) {
				System.out.println("Estado: Success"  + "\n" + "Salida: Bgserver Successful");
				result = true;
			}else {
	...
	public InetSocketAddress getServerAddress(String serverNicknameOrSocketAddr) {
		...
		if (serverNicknameOrSocketAddr.contains(":")) { // Then it has to be a socket address (IP:port)
			...
			String[] estring = serverNicknameOrSocketAddr.split(":");
			try {
				fserverAddr = new InetSocketAddress(InetAddress.getByName(estring[0]), Integer.parseInt(estring[1]));
	...
	public boolean unregisterFileServer() {
		...
		if(directoryConnector.unregisterServerport()) {
			System.out.println("Estado: Success" + "\n"  + "Salida: Bgserver Successfuly Disconnected");
			result = true;
		}else {
			System.out.println("Estado: Failure" + "\n" + "Salida: Bgserver Failed Disconnection");
		}
		return result;
	}
