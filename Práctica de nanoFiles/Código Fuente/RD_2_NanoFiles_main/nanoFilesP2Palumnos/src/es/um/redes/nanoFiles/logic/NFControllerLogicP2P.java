...
public class NFControllerLogicP2P {
	...
	protected boolean backgroundServeFiles() {
		...
		if (bgFileServer != null) {
			System.out.println("* Server is already serving files in the background");
			return false;
		}
		...
		if (bgFileServer.getPort() <= 0 || bgFileServer.getPort() > 65535) {
			System.out.println("* Port number isnt valied");
			return false;
		}
		
		System.out.println("PUERTO: " + bgFileServer.getPort());
		return true;
	}
	...
	protected boolean downloadFileFromSingleServer(InetSocketAddress fserverAddr, String targetFileHash,String localFileName) {
		...
		File fichero = new File(localFileName);
		if (fichero.exists() ) {
			System.err.println("* Cannot start download - No file provided");
			return false;
		}
		
		if (fichero.canRead() ) {
			System.err.println("* Cannot start download - File can't be read");
			return false;
		}
		if (fichero.canWrite()) {
			System.err.println("* Cannot start download - File can't be written	");
			return false;
		}
		...
		try {
			...
			if(conector.downloadFile(targetFileHash, fichero)) {
				result = true;
			}
	...
	public int getServerPort() {
		/*
		 * DONE: Devolver el puerto de escucha de nuestro servidor de ficheros en
		 * segundo plano
		 */
